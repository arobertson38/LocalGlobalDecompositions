...
def anneal_dsm_score_estimation_extended(scorenet, samples, labels, sigmas, anneal_power=2., repeats=10):
    # adding to samples
    raise NotImplementedError("Andreas: This isn't quite working properly.")
    # maybe a memory problem? Repeated memory?
    samples = torch.cat([samples,]*repeats, dim=0)
    labels = torch.cat([labels,] * repeats, dim=0)
